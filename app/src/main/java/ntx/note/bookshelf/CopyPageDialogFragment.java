...
public class CopyPageDialogFragment extends CustomDialogFragment {
    ...
    private void copyCurrentPageToCurrentNote() {
        ...
        Global.getCurrentBook().cloneCurrentPageToNextPage();
        if (mActivity.getClass().getSimpleName().equals(NoteWriterActivity.class.getSimpleName())) {
            ...
            if (Global.getCurrentBook().currentPage().objectsDrawTimePredict() > PAGE_REDRAW_TIME_THRESHOLD) {
            ...
            ((NoteWriterActivity) mActivity).switchToPage(Global.getCurrentBook().currentPage());
        }
    }
    ...
    private class InitAllBookListAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            for (Book book : allBookList) {
                if (book.getUUID().equals(Global.getCurrentBook().getUUID())) {
                    allBookList.remove(book);
                    break;
                }
            }
            mBookshelfListAdapter = new BookshelfListAdapter(allBookList, Global.getCurrentBook().isLandscape());
    ...
    private class CopyPageToNoteAsyncTask extends AsyncTask<Void, Void, Void> {
        @Override
        protected Void doInBackground(Void... voids) {
            Page currentPage = Global.getCurrentBook().currentPage();
