...
import name.vbraun.view.write.GetMainPagePreviewBitmapAsyncTask;
...
public class NtxLauncherListItem extends RelativeLayout {
    ...
    public @interface NoteType {
        ...
        int NOTE_BOOK_LANDSCAPE = 4;
        int NOTE_BOOK_CHECKABLE_LANDSCAPE = 5;
    }
    ...
    private GetMainPagePreviewBitmapAsyncTask mGetMainPagePreviewBitmapAsyncTask;
    ...
    public void setType(int noteType, @NonNull String title) {
        ...
        switch (noteType) {
            ...
            case NoteType.NOTE_BOOK_LANDSCAPE:
                mTvTitle.setText(title);
                mBtnCheck.setVisibility(INVISIBLE);
                break;
            case NoteType.NOTE_BOOK_CHECKABLE:
            case NoteType.NOTE_BOOK_CHECKABLE_LANDSCAPE:
                mTvTitle.setText(title);
    ...
    public void updateIconPreview(Book book) {
        ...
        if (mGetMainPagePreviewBitmapAsyncTask != null && mGetMainPagePreviewBitmapAsyncTask.getStatus().equals(AsyncTask.Status.RUNNING)) {
            mGetMainPagePreviewBitmapAsyncTask.cancel(true);
        }

        mGetMainPagePreviewBitmapAsyncTask = new GetMainPagePreviewBitmapAsyncTask(book.getUUID(), 0);
        mGetMainPagePreviewBitmapAsyncTask.asyncTaskResult = new AsyncTaskResult<Bitmap>() {
            @Override
        ...
        if (book.isLandscape())
            mGetMainPagePreviewBitmapAsyncTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, 200, 200);
        else
            mGetMainPagePreviewBitmapAsyncTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, 170, 170);
    }
