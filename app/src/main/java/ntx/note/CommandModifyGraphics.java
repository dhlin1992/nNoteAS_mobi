...
public class CommandModifyGraphics extends Command {
    ...
    @Override
    public void execute() {
        UndoManager.getApplication().modify_graphics(getPageUuid(), graphics, newDataGraphics);
    }

    @Override
    public void revert() {
        UndoManager.getApplication().modify_graphics(getPageUuid(), graphics, backupDataGraphics);
