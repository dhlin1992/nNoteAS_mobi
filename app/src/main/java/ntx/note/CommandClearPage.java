...
public class CommandClearPage extends Command {
    ...
    @Override
    public void execute() {
        UndoManager.getApplication().remove_for_clear(getPageUuid());
    }

    @Override
    public void revert() {
        UndoManager.getApplication().add_for_clear_revert(getPageUuid(), strokes, lineArt, rectangleArt, ovalArt, triangleArt);
    }
