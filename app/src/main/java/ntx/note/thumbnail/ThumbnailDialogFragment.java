...
public class ThumbnailDialogFragment extends CustomDialogFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mCurrentBook = Global.getCurrentBook();
        mCurrentBookTagManager = mCurrentBook.getTagManager();
        mThumbnailListAdapter = new ThumbnailListAdapter(mCurrentBook.getPages(), 6);
    ...
    private void initView(View v) {
        ...
        mStarTagItem.setTagCheck(mCurrentBook.getFilter().contains(quickTag), false);
                ...
                public void onClick(View v) {
                    ...
                    if (referencePageIndex >= 0) {
                        if (locationBtn == 3 || locationBtn == 7)
                            insertPosition = mCurrentBook.getPageNumber(referencePage.getUUID());
                        else
                            insertPosition = mCurrentBook.getPageNumber(referencePage.getUUID()) - 1;
                    } else
                        insertPosition = mCurrentBook.getPageNumber(referencePage.getUUID());
            ...
            thumbnailItem.setOnItemClickListener(new ThumbnailItem.ItemClickListener() {
                ...
                public void onClick(Object viewTag) {
                    if (TopLayoutMode.SELECTION_MODE == mTopLayoutMode) {
                        ...
                        ((NoteWriterActivity) getActivity()).switchToPage((Page) viewTag);
                        dismiss();
                    }
                }
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.btn_exit_check_mode:
                    ...
                    mEtSearchTag.clearFocus();
                    break;
                case R.id.btn_exit_paste_or_move_mode:
                    updateTopLayoutView(TopLayoutMode.CHECK_MODE);
                    LinkedList<Page> searchTextBoxResultList = getPagesTextBoxContainsSearchStr(mEtSearchTag.getText().toString());
                    LinkedList<Page> searchTagResultList = mCurrentBook.getUnionFilteredPages(mStarTagItem.isTagCheck());
                    mThumbnailListAdapter.updateList(getUnionList(searchTextBoxResultList, searchTagResultList));
    ...
    private int getPageIndex(Page page) {
        return mCurrentBook.getPageNumber(page.getUUID());
    }
    ...
    private void deleteSelectedPages() {
        ...
        mCurrentBook.filterChanged();
        mEtSearchTag.setText("");
        mStarTagItem.setTagCheck(false, true);
    ...
    private LinkedList<Page> getPagesTextBoxContainsSearchStr(String searchStr) {
        LinkedList<Page> resultPages = new LinkedList<>();
        if (searchStr.isEmpty())
            resultPages = mCurrentBook.getPages();
        else {
            for (Page page : mCurrentBook.getPages()) {
                for (TextBox textBox : page.textBoxes) {
                    String textBoxStrLowerCase = textBox.getTextStr().toLowerCase();
                    if (textBoxStrLowerCase.contains(searchStr.toLowerCase())) {
    ...
    private LinkedList<Page> getUnionList(LinkedList<Page> firstList, LinkedList<Page> secondList) {
        LinkedList<Page> outputList = new LinkedList<>();
        for (Page page : mCurrentBook.getPages()) {
            boolean inFirstList = firstList.contains(page);
            boolean inSecondList = secondList.contains(page);
    ...
    public void dismiss() {
        ...
        ((NoteWriterActivity) getActivity()).switchToPage(mCurrentBook.currentPage());
    }
}
