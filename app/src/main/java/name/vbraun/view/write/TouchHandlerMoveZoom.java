...
public class TouchHandlerMoveZoom extends TouchHandlerABC {
	...
	protected boolean onTouchEvent(MotionEvent event) {
		...
		if (action == MotionEvent.ACTION_MOVE) {
			...
			view.invalidate(PenEventNTX.UPDATE_MODE_PARTIAL_A2_WITH_DITHER);
			if (fingerId2 == -1) {  // move
...
//				view.centerAndFillScreen(newX1, newY1);
			...
			if (fingerId1 == event.getPointerId(0)) {
				view.invalidate(PenEventNTX.UPDATE_MODE_GLOBAL_RESET);
			}
