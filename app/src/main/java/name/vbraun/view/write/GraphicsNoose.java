...
public class GraphicsNoose extends GraphicsControlPoint {
    ...
    protected GraphicsNoose(Transformation transform, float x, float y) {
        super(Tool.NOOSE);
    ...
    protected GraphicsNoose(Transformation transform, RectF nooseRectF){
        super(Tool.NOOSE);
        setTransform(transform);

        cp_tl = new ControlPoint(transform, nooseRectF.left, nooseRectF.top);
        cp_bl = new ControlPoint(transform, nooseRectF.left, nooseRectF.bottom);
        cp_tr = new ControlPoint(transform, nooseRectF.right, nooseRectF.top);
        cp_br = new ControlPoint(transform, nooseRectF.right, nooseRectF.bottom);
        cp_c = new ControlPoint(transform, nooseRectF.centerX(), nooseRectF.centerY());
        controlPoints.add(cp_c);
        setPen();
    }

    protected GraphicsNoose(Transformation transform) {
        super(Tool.POLYGON_NOOSE);
        setTransform(transform);
        int x = 0, y = 0;
        cp_tl = new ControlPoint(transform, x, y);
        cp_bl = new ControlPoint(transform, x, y);
        cp_tr = new ControlPoint(transform, x, y);
        cp_br = new ControlPoint(transform, x, y);
        cp_c = new ControlPoint(transform, x, y);
        /**
         * Noose has only the center control point.
         */
