...
public class Stroke extends Graphics {
    ...
    public static final int CHARCOAL = Global.grey_6;
...
//		Assert.assertTrue("Pen type is not actual pen.", pen_type == Tool.FOUNTAINPEN || pen_type == Tool.PENCIL);
    ...
    @Override
    public boolean isCenterControlPoint(GraphicsControlPoint.ControlPoint controlPoint) {
        return false;
    }
...
//			drawFountainpenWithCubicBezier(c);
    ...
    public void drawOutLine(Canvas c) {
        ...
        if (N <= 2 || (tool == Tool.PENCIL && !zoom)) {
            ...
            bitmapShader = getShaderByPenColor(Color.WHITE);
            mPen.setShader(bitmapShader);
            drawWhiteWithStraightLineUSingBrushFountain(c);
        } else if (tool == Tool.FOUNTAINPEN) {
            /**
             * Draw Black
             */
...
//        mPen.setShader(shader);
    ...
    public void drawThumbnail(Canvas c) {
        ...
        if (N <= 2 || (tool == Tool.PENCIL && !zoom))
            ...
            drawThumbWithStraightLineUSingBrushFountain(c);
        else if (tool == Tool.FOUNTAINPEN)
            drawThumbWithStraightLineUSingBrushFountain(c);
    }
...
//		Log.d("test","stroke"+ getScaledPenThickness() * Global.BRUSH_THICKNESS_WEIGHT);
        for (int i = 1; i < N; i++) {
            // dango 20190621: charcoal pen set shader again to override previous 5 color setting
            if (pen_color == Stroke.CHARCOAL) {
                BitmapShader mBitmapShader = getShaderByPenColorSpecialEffect(pen_color, x0, y0);
                mPen.setShader(mBitmapShader);
            }
    ...
    private void drawWhiteWithStraightLineUSingBrushFountain(Canvas c) {
...
//		x0 = position_x[0] * scale + offset_x;
    ...
    private void drawWhiteWithStraightLine(Canvas c) {
        int currentThickness = (int) (getPencilThickness());
        mPen.setStrokeWidth(currentThickness);
...
//		x0 = position_x[0] * scale + offset_x;
    ...
    private BitmapShader getShaderByPenColorSpecialEffect(int pen_color, float startX, float startY) {
        ...
        int LSB = (int) startX % 2;
        int MSB = (int) startY % 2;
        MSB = MSB << 1;
        int shaderIndex = LSB + MSB;

        switch (pen_color) {
            case CHARCOAL:
                mBitmapShader = Global.getBitmapShader(shaderIndex);
                break;
            default:
                mBitmapShader = Global.getBitmapShader(0);
        }
        return mBitmapShader;
    }
...
//		x0 = position_x[0] * scale + offset_x;
    ...
    private void drawThumbWithStraightLineUSingBrushFountain(Canvas c) {
...
//		float norm_weight = 2.0f;	//artis, original value
    ...
    public ArrayList<Point> getPoints() {
        ...
        for (int i = 0; i < position_x.length; i++) {
            points.add(new Point(Math.round(position_x[i] * scale + offset_x),
                    Math.round(position_y[i] * scale + offset_y)));
        }
        return points;
    }
}
