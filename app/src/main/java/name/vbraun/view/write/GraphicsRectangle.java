...
//        pen.setShader(shader);
    ...
    void controlPointMoved(ControlPoint point, float newX, float newY) {
        ...
        if (point == cp_c) {
            ...
            cp_c.y = rectF.top + (rectF.bottom - rectF.top) / 2;
        }
    }
    ...
    public Rect getOutLineBounding() {
        int drawOutLine = (int) Math.ceil((float) pen_thickness / 2);
        return new Rect(rect.left - drawOutLine, rect.top - drawOutLine, rect.right + drawOutLine, rect.bottom + drawOutLine);
    }
    ...
    public ArrayList<Point> getPoints() {
        ...
        points.add(new Point(Math.round(cp_tl.screenX()), Math.round(cp_tl.screenY())));
        points.add(new Point(Math.round(cp_tr.screenX()), Math.round(cp_tr.screenY())));
        points.add(new Point(Math.round(cp_bl.screenX()), Math.round(cp_bl.screenY())));
        points.add(new Point(Math.round(cp_br.screenX()), Math.round(cp_br.screenY())));
        return points;
    }
}
